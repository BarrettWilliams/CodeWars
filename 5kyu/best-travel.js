...
function chooseBestSum(t, k, ls) {
  ...
  function generateCombinations(start, currentGroup) {
    if (currentGroup.length === k) {
      group.push([...currentGroup]);
      return;
    }
    for (let i = start; i < ls.length; i++) {
      currentGroup.push(ls[i]);
      generateCombinations(i + 1, currentGroup);
      currentGroup.pop(); 
    }
  }
  ...
  let sum = group.map(x =>  x.reduce((a,b) => a+b,0))
  let fit = sum.filter(x => x <= t)
  fit.sort((a,b) => b-a)
let num = fit[0]
if(fit.length === 0){
  return null
